...
class InteractiveObservablesFunction:
    ...
    def __init__(self, app, is_2d=False):
        ...
        # Flag to track if the potential has changed
        self.potential_changed = False
        self.last_applied_potential_type = "none"  # Track the last applied potential type
  ...
  Maps probability density |ψ(x,y)|² to color.
    ...
    def update_preview(self, *args):
        ...
        if self.is_2d:
            ...
            # Check if potential type has changed
            if potential_type != self.last_applied_potential_type:
                self.potential_changed = True
    ...
    def apply_to_simulation(self):
        ...
        if self.is_2d:
            sim = self.app.sim_2d
            # Ensure we have a valid potential function
            if self.last_potential_func_2d is None:
                # Default to no potential if none is set
                self.last_potential_func_2d = lambda x, y: np.zeros_like(x)
            ...
            sim = self.app.sim_1d
            # Ensure we have a valid potential function
            if self.last_potential_func_1d is None:
                # Default to no potential if none is set
                self.last_potential_func_1d = lambda x: np.zeros_like(x)
        ...
        # Reset the potential changed flag and update the last applied type
        self.potential_changed = False
        self.last_applied_potential_type = self.potential_var.get()
    ...
    def update_plot(self):
        ...
        if self.is_2d:
            ...
            # Update transmission/reflection plot (1D only) - with safety checks
            if hasattr(self, 'line_trans') and hasattr(self, 'line_refl'):
    ...
    def reset_history(self):
        ...
        if self.is_2d:
            ...
            # Transmission/reflection plots don't exist in 2D mode
        else:
            ...
            # Only reset transmission/reflection if they exist (1D only)
            if hasattr(self, 'line_trans'):
                self.line_trans.set_data([], [])
            if hasattr(self, 'line_refl'):
    ...
    def reset(self):
        ...
        # Force update of the potential function based on current UI selection
        self.update_preview()
        
        # Apply the current parameters to the main simulation
        self.apply_to_simulation()
        
        # Ensure the simulation is properly reset to time 0
        if self.is_2d:
            self.app.sim_2d.time = 0.0
            self.app.sim_2d.time_points = []
            self.app.sim_2d.energy_expectation = []
            self.app.sim_2d.kinetic_energy = []
            self.app.sim_2d.potential_energy = []
            self.app.sim_2d.position_expectation_x = []
            self.app.sim_2d.position_expectation_y = []
            self.app.sim_2d.uncertainty_x = []
            self.app.sim_2d.uncertainty_y = []
            self.app.sim_2d.uncertainty_px = []
            self.app.sim_2d.uncertainty_py = []
            self.app.sim_2d.heisenberg_x = []
            self.app.sim_2d.heisenberg_y = []
        else:
            self.app.sim_1d.time = 0.0
            self.app.sim_1d.time_points = []
            self.app.sim_1d.energy_expectation = []
            self.app.sim_1d.kinetic_energy = []
            self.app.sim_1d.potential_energy = []
            self.app.sim_1d.position_expectation = []
            self.app.sim_1d.momentum_expectation = []
            self.app.sim_1d.uncertainty_x = []
            self.app.sim_1d.uncertainty_p = []
            self.app.sim_1d.heisenberg_product = []
            self.app.sim_1d.probability_current_history = []
        
        # Reset the potential changed flag since we just applied the current potential
        self.potential_changed = False
        self.last_applied_potential_type = self.potential_var.get()
