...
class QuantumApp:
    ...
    def start_simulation_1d(self):
        ...
        if self.current_function_1d and not self.animation_running_1d:
            # Check if this is Interactive Observables function and potential has changed
            if (hasattr(self.current_function_1d, 'potential_changed') and 
                self.current_function_1d.potential_changed):
                result = messagebox.askyesno(
                    "Potential Changed", 
                    "The potential type has been changed but not applied to the simulation.\n\n"
                    "Would you like to reset the simulation first to apply the new potential?\n\n"
                    "Click 'Yes' to reset and apply the new potential, or 'No' to continue with the old potential.",
                    icon='warning'
                )
                if result:  # User chose to reset
                    self.reset_simulation_1d()
                # If user chose 'No', continue with the old potential
    ...
    def start_simulation_2d(self):
        ...
        if self.current_function_2d and not self.animation_running_2d:
            # Check if this is Interactive Observables function and potential has changed
            if (hasattr(self.current_function_2d, 'potential_changed') and 
                self.current_function_2d.potential_changed):
                result = messagebox.askyesno(
                    "Potential Changed", 
                    "The potential type has been changed but not applied to the simulation.\n\n"
                    "Would you like to reset the simulation first to apply the new potential?\n\n"
